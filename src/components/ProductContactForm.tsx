...
import type {
  IFormDataProductContactForm,
  IFocusedProductContactForm,
} from "../types/types";
...
const ProductContactForm: React.FC<ProductContactFormProps> = ({
  productName = "",
}) => {
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {[
              ...
              },
              {
                ...
                type: "text",
                error: "Please enter the product name (at least 2 characters)",
                delay: 0.4,
                colSpan: true,
              },
              {
              ...
              <motion.div
                key={field.name}
                className={`mb-4 ${
                  field.colSpan ? "col-span-1 md:col-span-2" : ""
                }`}
                initial={{
                  x: field.colSpan
                    ? 0
                    : field.name === "email" || field.name === "companyName"
                    ? 10
                    : -10,
                  opacity: 0,
                  y: field.colSpan ? 10 : 0,
                }}
              ...
              >
                <TextField
                  ...
                  name={field.name}
                  value={
                    formData[field.name as keyof IFormDataProductContactForm]
                  }
                  onChange={handleChange}
                  onFocus={() =>
                    handleFocus(field.name as keyof IFocusedProductContactForm)
                  }
                  onBlur={() =>
                    handleBlur(field.name as keyof IFocusedProductContactForm)
                  }
                  ...
                  InputProps={{
                    startAdornment: (
                      <InputAdornment position="start">
                        <field.icon
                          className={`${
                            focused[
                              field.name as keyof IFocusedProductContactForm
                            ] ||
                            formData[
                              field.name as keyof IFormDataProductContactForm
                            ]
                              ? "text-blue-600"
                              : "text-gray-400"
                          }`}
                    ...
                    sx: {
                      ...
                      "& .MuiOutlinedInput-notchedOutline": {
                        borderColor:
                          hasSubmitted && !isFieldValid(field.name)
                            ? "red"
                            : "gray",
                        transition:
                  ...
                  helperText={
                    hasSubmitted && !isFieldValid(field.name) ? field.error : ""
                  }
                  sx={{
